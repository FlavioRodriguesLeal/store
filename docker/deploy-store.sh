...
if [ $(id -u) -eq 0 ]; then
  ...
  docker stop store-api
	...
	docker run -d --name mongodb-store --network store-network -p 27018:27017 -e MONGODB_USERNAME=store -e MONGODB_PASSWORD=storepass -e MONGODB_DATABASE=store bitnami/mongodb:latest
	if [ $? -eq 0 ]; then
